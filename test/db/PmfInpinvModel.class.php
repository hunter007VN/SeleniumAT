 ...
 * @package entity
 */
class PmfInpinvModel extends Db2PhpEntityBase implements Db2PhpEntityModificationTracking {
	private static $CLASS_NAME='PmfInpinvModel';
	const SQL_IDENTIFIER_QUOTE='`';
	const SQL_TABLE_NAME='pmf_inpinv';
	const SQL_INSERT='INSERT INTO `pmf_inpinv` (`id`,`itemmasterid`,`ssn`,`locid`,`partnerid`,`qty`,`invaction`,`dtm`,`usrid`,`refid`,`jobid`,`caseid`,`bounce`,`palletid`,`coo`) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)';
	const SQL_INSERT_AUTOINCREMENT='INSERT INTO `pmf_inpinv` (`itemmasterid`,`ssn`,`locid`,`partnerid`,`qty`,`invaction`,`dtm`,`usrid`,`refid`,`jobid`,`caseid`,`bounce`,`palletid`,`coo`) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)';
	const SQL_UPDATE='UPDATE `pmf_inpinv` SET `id`=?,`itemmasterid`=?,`ssn`=?,`locid`=?,`partnerid`=?,`qty`=?,`invaction`=?,`dtm`=?,`usrid`=?,`refid`=?,`jobid`=?,`caseid`=?,`bounce`=?,`palletid`=?,`coo`=? WHERE `id`=?';
	const SQL_SELECT_PK='SELECT * FROM `pmf_inpinv` WHERE `id`=?';
	const SQL_DELETE_PK='DELETE FROM `pmf_inpinv` WHERE `id`=?';
	const FIELD_ID=-346783443;
	const FIELD_ITEMMASTERID=502195362;
	const FIELD_SSN=2134625340;
	const FIELD_LOCID=-1626006775;
	const FIELD_PARTNERID=1498352529;
	const FIELD_QTY=2134623460;
	const FIELD_INVACTION=1148702997;
	const FIELD_DTM=2134610955;
	const FIELD_USRID=-1617561507;
	const FIELD_REFID=-1620760676;
	const FIELD_JOBID=-1627854778;
	const FIELD_CASEID=863278941;
	const FIELD_BOUNCE=847647130;
	const FIELD_PALLETID=1670032205;
	const FIELD_COO=2134609841;
	...
	private static $FIELD_TYPES=array(
		...
		self::FIELD_INVACTION=>array(Db2PhpEntity::JDBC_TYPE_INTEGER,10,0,false),
		self::FIELD_DTM=>array(Db2PhpEntity::JDBC_TYPE_TIMESTAMP,19,0,true),
		self::FIELD_USRID=>array(Db2PhpEntity::JDBC_TYPE_INTEGER,10,0,false),
	 ...
	 * Query by Example.
	 *
	 * Match by attributes of passed example instance and return matched rows as an array of PmfInpinvModel instances
	 *
	 * @param PDO $db a PDO Database instance
	 * @param PmfInpinvModel $example an example instance defining the conditions. All non-null properties will be considered a constraint, null values will be ignored.
	 * @param boolean $and true if conditions should be and'ed, false if they should be or'ed
	 * @param array $sort array of DSC instances
	 * @return PmfInpinvModel[]
	 */
	public static function findByExample(PDO $db,PmfInpinvModel $example, $and=true, $sort=null) {
	 ...
	 *
	 * Will return matched rows as an array of PmfInpinvModel instances.
	 *
	 * @param PDO $db a PDO Database instance
	...
	/**
	 * Will execute the passed statement and return the result as an array of PmfInpinvModel instances
	 *
	...
	/**
	 * returns the result as an array of PmfInpinvModel instances without executing the passed statement
	 *
	 * @param PDOStatement $stmt
	 * @return PmfInpinvModel[]
	 */
	...
	/**
	 * Execute select query and return matched rows as an array of PmfInpinvModel instances.
	 *
	...
	public function insertIntoDatabase(PDO $db) {
		if (null===$this->getId()) {
			...
			$stmt->bindValue(1,$this->getItemmasterid());
			$stmt->bindValue(2,$this->getSsn());
			$stmt->bindValue(3,$this->getLocid());
			$stmt->bindValue(4,$this->getPartnerid());
			$stmt->bindValue(5,$this->getQty());
			$stmt->bindValue(6,$this->getInvaction());
			$stmt->bindValue(7,$this->getDtm());
			$stmt->bindValue(8,$this->getUsrid());
			$stmt->bindValue(9,$this->getRefid());
			$stmt->bindValue(10,$this->getJobid());
			$stmt->bindValue(11,$this->getCaseid());
			$stmt->bindValue(12,$this->getBounce());
			$stmt->bindValue(13,$this->getPalletid());
			$stmt->bindValue(14,$this->getCoo());
		} else {
	...
	/**
	 * Fetch PmfInvModel's which this PmfInpinvModel references.
	 * `pmf_inpinv`.`id` -> `pmf_inv`.`itemid`
	 *
	 * @param PDO $db a PDO Database instance
	 * @param array $sort array of DSC instances
	 * @return PmfInvModel[]
	...
	public function fetchPmfInvModelCollection(PDO $db, $sort=null) {
		$filter=array(PmfInvModel::FIELD_ITEMID=>$this->getId());
		return PmfInvModel::findByFilter($db, $filter, true, $sort);
	}

	/**
	 * Fetch PmfInvhisModel's which this PmfInpinvModel references.
	 * `pmf_inpinv`.`id` -> `pmf_invhis`.`itemid`
	 *
	 * @param PDO $db a PDO Database instance
	 * @param array $sort array of DSC instances
	...
	public function fetchPmfInvhisModelCollection(PDO $db, $sort=null) {
		$filter=array(PmfInvhisModel::FIELD_ITEMID=>$this->getId());
		return PmfInvhisModel::findByFilter($db, $filter, true, $sort);
	}

	/**
	 * Fetch PmfOutinvModel's which this PmfInpinvModel references.
	 * `pmf_inpinv`.`id` -> `pmf_outinv`.`itemid`
	 *
	 * @param PDO $db a PDO Database instance
	 * @param array $sort array of DSC instances
	...
	public function fetchPmfOutinvModelCollection(PDO $db, $sort=null) {
		$filter=array(PmfOutinvModel::FIELD_ITEMID=>$this->getId());
		return PmfOutinvModel::findByFilter($db, $filter, true, $sort);
	}

	/**
	 * Fetch CasepartModel which references this PmfInpinvModel. Will return null in case reference is invalid.
	 * `pmf_inpinv`.`caseid` -> `casepart`.`id`
	 *
	 * @param PDO $db a PDO Database instance
	 * @param array $sort array of DSC instances
	...
	/**
	 * Fetch PmfInvactionModel which references this PmfInpinvModel. Will return null in case reference is invalid.
	 * `pmf_inpinv`.`invaction` -> `pmf_invaction`.`id`
	 *
	 * @param PDO $db a PDO Database instance
	 * @param array $sort array of DSC instances
	...
	/**
	 * Fetch ItemmasterModel which references this PmfInpinvModel. Will return null in case reference is invalid.
	 * `pmf_inpinv`.`itemmasterid` -> `itemmaster`.`id`
	 *
	 * @param PDO $db a PDO Database instance
	 * @param array $sort array of DSC instances
	...
	/**
	 * Fetch PmfJobModel which references this PmfInpinvModel. Will return null in case reference is invalid.
	 * `pmf_inpinv`.`jobid` -> `pmf_job`.`id`
	 *
	 * @param PDO $db a PDO Database instance
	 * @param array $sort array of DSC instances
	 * @return PmfJobModel
	...
	/**
	 * Fetch LocationModel which references this PmfInpinvModel. Will return null in case reference is invalid.
	 * `pmf_inpinv`.`locid` -> `location`.`id`
	 *
	 * @param PDO $db a PDO Database instance
	 * @param array $sort array of DSC instances
	...
	/**
	 * Fetch OrderdetailModel which references this PmfInpinvModel. Will return null in case reference is invalid.
	 * `pmf_inpinv`.`refid` -> `orderdetail`.`id`
	 *
	 * @param PDO $db a PDO Database instance
	 * @param array $sort array of DSC instances
	...
	/**
	 * Fetch PartnerModel which references this PmfInpinvModel. Will return null in case reference is invalid.
	 * `pmf_inpinv`.`partnerid` -> `partner`.`id`
	 *
	 * @param PDO $db a PDO Database instance
	 * @param array $sort array of DSC instances
	 * @return PartnerModel
	...
	/**
	 * Fetch UsersModel which references this PmfInpinvModel. Will return null in case reference is invalid.
	 * `pmf_inpinv`.`usrid` -> `users`.`id`
	 *
	 * @param PDO $db a PDO Database instance
	 * @param array $sort array of DSC instances
	 * @return UsersModel
	...
	public function toDOM() {
		return self::hashToDomDocument($this->toHash(), 'PmfInpinvModel');
	}

	/**
	 * get single PmfInpinvModel instance from a DOMElement
	 *
	...
	/**
	 * get all instances of PmfInpinvModel from the passed DOMDocument
	 *
	...
	public static function fromDOMDocument(DOMDocument $doc) {
		$instances=array();
		foreach ($doc->getElementsByTagName('PmfInpinvModel') as $node) {
